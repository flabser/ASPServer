...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="header-view">	
		<div id="header-view">
			<span style="float:left">
				...
				<font style="font-size:1.3em; vertical-align:27px; color:#555555">Администратор ЕИС "Город"</font>
			</span>
	...
	<xsl:template name="outline-section-state">
		<script>
			if($.cookie("EISAdmin_<xsl:value-of select='@id'/>")){
				$("#<xsl:value-of select='@id'/>").css("display",$.cookie("EISAdmin_<xsl:value-of select='@id'/>"))
				if($.cookie("EISAdmin_<xsl:value-of select='@id'/>") == "none"){
	...
	<xsl:template name="outline-menu-view">	
		<div id="outline">
			<div id="outline-container" style="width:380px; padding-top:10px">
							...
							<xsl:for-each select="entry">
								...
								<xsl:for-each select="entry">
									<div class="entry treeentry" style="width:auto; padding:3px 0px 3px 0px; border:1px solid #F9F9F9;">
										...
										<div class="viewlink">
											<div style="width:65px; height:20px; float:left"></div>
											<div style="margin-left:15px ">
												<img src="/SharedResources/img/classic/1/doc_view.png" style=" cursor:pointer"/>
												<a href="{@url}" style="width:90%; vertical-align:top; text-decoration:none !important" title="{@hint}">
													<font class="viewlinktitle">	
		...
		<div id="resizer" style="position:absolute; top: 80px; left:386px; background:#E6E6E6; width:12px; bottom:0px; border-radius: 0 6px 6px 0; border: 1px solid #adadad; border-left: ; cursor:pointer" onclick="javascript: closepanel()">
					...
					<xsl:when test="$maxpage - $currentpage &lt; 7">
						<xsl:if test="$i != $n+1">
							<xsl:if test="$z &lt; $maxpage + 1">
								<td>
									<a href="">
										<xsl:attribute name="href">javascript:window.location.href="Provider?type=view&amp;id=<xsl:value-of select='@id'/>&amp;page=<xsl:value-of select='$z'/>"</xsl:attribute>
						...
						<xsl:if test="$currentpage != $maxpage">
							<xsl:if test="$i = $n+1">
		 						<td>
     								<a href="">
     									<xsl:attribute name="href">javascript:window.location.href="Provider?type=view&amp;id=<xsl:value-of select='@id'/>&amp;page=<xsl:value-of select='$nextpage'/>"</xsl:attribute>
      									<font style="font-size:12px"> > </font>
      								</a>
      							</td>
       							<td>
       								<a href="">
       									<xsl:attribute name="href">javascript:window.location.href="Provider?type=view&amp;id=<xsl:value-of select='@id'/>&amp;page=<xsl:value-of select='$maxpage'/>"</xsl:attribute>
							...
							<xsl:when test="$currentpage &lt; 7">
								...
								<xsl:if test="$i != $n+1">
									<xsl:if test="$i &lt; $maxpage + 1">
										<td>
											<a href="">
												<xsl:attribute name="href">javascript:window.location.href="Provider?type=view&amp;id=<xsl:value-of select='@id'/>&amp;page=<xsl:value-of select='$i'/>"</xsl:attribute>
												<font style="font-size:12px">
								...
								<xsl:if test="$i != $n+1">
									<xsl:if test="$i &lt; $maxpage + 1">
										<xsl:if test="$startnum !=0">
											<td>
												<a href="">
													<xsl:attribute name="href">javascript:window.location.href="Provider?type=view&amp;id=<xsl:value-of select='@id'/>&amp;page=<xsl:value-of select='$startnum'/>"</xsl:attribute>
													<font style="font-size:12px">
