...
function pickListSingleOk(docid){
	...
	if (queryOpt.fieldname == "responsiblesection"){
		...
		$("#frm").append("<input type='hidden' name='coordBlock'  id='coordBlockSign' value='new`par`0`"+docid+"`367'>")
	}
	...
	if (queryOpt.fieldname == 'executor'){
		...
		$('#intexectable').append("<tr>" +
				...
				"<td id='switchControl"+num+"'><a href='javascript:controlOff("+num+")'><img  title='Снять с контроля' src='/SharedResources/img/classic/exec_control.gif'/><a/></td>" +
		"</tr>");
	}else{
	...
	if (queryOpt.fieldname == "executor"){
		for( var i = 0; i < $("#executorsColl").children("input[type=hidden]").length; i ++ ){
			...
			$('#intexectable').append("<tr>" +
				...
				"<td id='controlOffDate"+num+"'></td>" +
				"<td id='idCorrControlOff"+num+"'></td>" +
				"<td id='switchControl"+num+"'><a href='javascript:controlOff("+num+")'><img  title='Снять с контроля' src='/SharedResources/img/classic/icons/eye_delete.png'/><a/></td>" +
			"</tr>");
		}
	}else{
		if(queryOpt.fieldname == 'corr' || queryOpt.fieldname == 'recipient' && queryOpt.queryname == 'corrcat'){
...
function enableblockform(){
	...
	$('#blockWindow').css('width',$(window).width()); 
	$('#blockWindow').css('height',$(window).height());
	...
	$.ajax({
		...
		success:function (data){
			if (data.match("login") && data.match("password")){
				...
				if	(queryOpt.queryname == 'workdocsigners'){
					$("#btnChangeView").remove();
				}
				if	(queryOpt.queryname == 'responsiblesection'){
					$("#btnChangeView").remove();
				}
				if	(queryOpt.fieldname == 'executor'){
		...
		success:function (data){
			if (data.match("login") && data.match("password")){
				...
				$("input[name=members]").each(function(){
					...
					$("input[name=chbox][id="+$(this).val()+"]").parent("div").removeAttr("ondblclick").removeAttr("onmouseover").removeAttr("onmouseout").css("color","#ccc");
				})
			}
		},
		error:function (xhr, ajaxOptions, thrownError){
...
function addMemberBtnOk(){
	...
	for( var i = 0; i < chBoxes.length; i ++ ){
		if (chBoxes[i].checked && $(chBoxes[i]).attr("id") != ''){ 
			...
			k=k+1;
			$("#"+ queryOpt.tablename).append("<tr><td style='width:600px;' class='td_editable'>"+chBoxes[i].value+"<input type='hidden' name='"+queryOpt.fieldname+"' id='"+queryOpt.fieldname+"' value='"+chBoxes[i].id+"'><img style='width:15px; height:15px; margin-right:3px; margin-top:1px; float:right; cursor:pointer' src='/SharedResources/img/iconset/cross.png' onclick='delmember(this)'></td></tr>");
		//$("#"+ queryOpt.formname).append("<input type='hidden' name='"+queryOpt.fieldname+"' id='"+queryOpt.fieldname+"' value='"+chBoxes[i].id+"'>")
		}
	}
	if (k>0){
