...
//			Assert.assertTrue(response != null && response.length > 0);
            ...
            for (FullResponse_ fullResponse_ : response) {
                ...
                def regplace = new _Tag("regplace"+i, (String)fullResponse_.getCommonInfo().getCapableCourtName())
                tag.addTag(regplace)
                //место регистрации
...
//          номер Удостоверения
                def idnumber = new _Tag("idnumber"+i, (String)fullResponse_.getCommonInfo().getCapableNumber())
                tag.addTag(idnumber)
